...
class TestIntegration:
    ...
    async def test_complete_user_workflow(
        ...
        response = test_client.post(
            f"/api/v1/users/{test_user.id}/matrix/{test_skill.id}",
            json=matrix_data,
        ...
        response = test_client.get(
            f"/api/v1/users/{test_user.id}/matrix", headers=auth_headers
        )
        assert response.status_code == 200
        ...
        user_skill = UserSkills(
            user_id=test_user.id, skill_id=test_skill.id, grade_id=None
        )
        test_session.add(user_skill)
        await test_session.commit()

        response = test_client.post(
            f"/api/v1/users/{test_user.id}/notifications", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    async def test_admin_workflow(
        self, test_client: TestClient, admin_auth_headers, mock_admin_user, test_session
    ):
        ...
        response = test_client.post(
            "/api/v1/skills", json=skill_data, headers=admin_auth_headers
        )
        assert response.status_code == 200
        skill = response.json()
        ...
        response = test_client.post(
            "/api/v1/grades", json=grade_data, headers=admin_auth_headers
        )
        assert response.status_code == 200
        grade = response.json()
        ...
        response = test_client.put(
            "/api/v1/configuration", json=config_data, headers=admin_auth_headers
        )
