...
class TestAnalyticsRouter:
    ...
    def test_get_analytics_forbidden_non_admin(self, test_client: TestClient, auth_headers):
        response = test_client.get("/api/v1/analytics", headers=auth_headers)
        assert response.status_code == 403

    def test_get_analytics_success(self, test_client: TestClient, admin_auth_headers, mock_admin_user):
